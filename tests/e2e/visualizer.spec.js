...
async function expectNoA11yViolations(page) {
  ...
  // reports semi-transparent text, so wait for the visualizer's animations to
  // settle. The site theme runs infinite animations, which never finish.
  await page.evaluate(() => Promise.all(
    document.querySelector('.brakeman-visualizer')
      .getAnimations({ subtree: true })
      .filter(animation => animation.effect && animation.effect.getComputedTiming().endTime !== Infinity)
